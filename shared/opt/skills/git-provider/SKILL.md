---
name: git-provider
description: "Forgejo and GitHub access. Uses the USER's personal token for privileged operations (invite collaborators, setup webhooks, branch protection). Read operations use the agent token. Supports both Forgejo and GitHub App providers."
...
# git-provider — Git operations

## Token used

- **Read operations** (issues, PRs, files): agent token (automatic)
- **Privileged operations** (invite, webhook, protect branch, delete branch): user's PERSONAL token via `/token set`
