...
async function push(ownerRepo) {
  ...
  // devcontainer.json is generated by the devcontainer-init skill after BMAD
  // Only copy a minimal placeholder if the skill hasn't run yet
  if (!fs.existsSync(`${cloneDir}/.devcontainer/devcontainer.json`)) {
    fs.writeFileSync(`${cloneDir}/.devcontainer/devcontainer.json`,
      JSON.stringify({ name: name, image: 'mcr.microsoft.com/devcontainers/base:alpine',
        customizations: { vscode: { extensions: [] } },
        postCreateCommand: '# Generated by /spec init — run /devcontainer init to customize' }, null, 2));
  }
