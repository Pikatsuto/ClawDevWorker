---
name: loop-detect
description: "Detects and interrupts infinite loops: same error repeated, same file modified without progress, same command failing in a loop. Use this skill AUTOMATICALLY when you feel like you're going in circles, redoing the same thing without making progress, or have failed more than 2 times on the same task. In VSCode: ask the human for help. In headless worker: signal FAIL with a detailed summary."
...
3. **Tests still fail** after 2 correction → test → correction cycles
4. **You re-read the same instructions** looking for something that isn't there
5. **You generate code that reproduces an error** you had already fixed
6. **The diff of your changes is empty** after having "fixed" something

## Automatic detection procedure

Before each new correction attempt, check:
...
I tried N times to fix [problem description] without success.
...
**Where I'm stuck:**
[precise description of the persistent problem]
...
**You NEVER attempt a 4th correction of the same problem.**
