---
name: loop-detect
description: "Detects and interrupts infinite loops: same error repeated, same file modified without progress, same command failing in a loop. Use this skill AUTOMATICALLY when you feel like you are going in circles, redoing the same thing without making progress, or have failed more than 2 times on the same task. In VSCode: ask the human for help. In headless worker: report FAIL with a detailed summary."
...
5. **You generate code that reproduces an error** that you had already fixed
...
**Where I'm stuck:**
[precise description of the problem that persists]
...
**You NEVER try a 4th correction attempt for the same problem.**
