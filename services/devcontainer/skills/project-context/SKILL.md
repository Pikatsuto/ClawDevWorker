...
description: "Loads the context of a project to work on it in any environment. /project select loads the memory and conventions. /project status shows ongoing issues and PRs. /project code loads the code in read-only mode (explicit). Shares context between chat, VSCode, and worker."
...
| `/project status` | Ongoing issues and PRs (git API) |
