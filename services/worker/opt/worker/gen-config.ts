/**
 * gen-config.ts — Generates openclaw.json for the worker (11 specialist roles)
 */
...
const stagedMode = process.env.STAGED_MODE !== 'false';
...
// Used by entrypoint for validation
export const SPECIALIST_ROLES = [
  'architect', 'frontend', 'backend', 'fullstack', 'devops',
  'qa', 'doc', 'marketing', 'design', 'product', 'bizdev',
] as const;

const loadSpecialistPrompt = (r: string): string => {
  const filePath = join(SPECIALISTS_DIR, `${r}.md`);
...
// ── Read git_flow config from rules.yaml ────────────────────────────────────

let gitFlowStrategy = 'trunk';
let gitFlowTarget = 'main';
try {
  const rulesPath = join(repoWorkspace, '.coderclaw/rules.yaml');
  if (existsSync(rulesPath)) {
    const rulesContent = readFileSync(rulesPath, 'utf8');
    const stratMatch = rulesContent.match(/strategy:\s*(\w+)/);
    const targetMatch = rulesContent.match(/target_branch:\s*(\S+)/);
    if (stratMatch?.[1]) gitFlowStrategy = stratMatch[1];
    if (targetMatch?.[1]) gitFlowTarget = targetMatch[1];
  }
} catch { /* silent */ }
...
const getAllowedTools = (r: string): string[] => {
  ...
  const toolMap: Record<string, string[]> = {
    ...
    fullstack: devTools,
    devops: devTools,
    security: [...reviewTools, 'file.write'],
    qa: [...reviewTools, 'file.write'],
...
const getEnabledSkills = (r: string): string[] => {
  const sharedSkills = [
